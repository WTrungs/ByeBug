...
import { xAxisTick, yAxisTick, tooltipContentStyle, tooltipCursor, legendWrapperStyle, barSize } from '../constants/chartConfig';
        ...
        <div className={styles.pageLayout}>
            ...
            <div className={styles.mainArea}>
                ...
                <div className={`topbar ${styles.topbar}`}>
                    <div>
                        <p className="topbar-title">THỐNG KÊ CÁ NHÂN</p>
                        <p className="topbar-welcome">
                            Chào mừng trở lại, <strong>{user?.username ?? 'Người dùng'}</strong>. Tiếp tục luyện tập nào!
                        </p>
                    </div>
                ...
                <div className={styles.content}>
                    ...
                    <div className={styles.twoColRow}>
                        ...
                        <div className={`${styles.card} ${styles.chartCard}`}>
                            ...
                            <ResponsiveContainer width="100%" height={240}>
                                <BarChart data={chartData} barSize={barSize} barGap={4}>
                                    ...
                                    <XAxis dataKey="day" tick={xAxisTick} axisLine={{ stroke: '#111', strokeWidth: 2 }} tickLine={false} />
                                    <YAxis tick={yAxisTick} axisLine={{ stroke: '#111', strokeWidth: 2 }} tickLine={false} />
                        ...
                        <div className={`${styles.card} ${styles.featuredCard}`}>
                            <span className={`${styles.sectionTitle} ${styles.sectionTitleBlock}`}>🔥 Bài tập đề xuất</span>
                            <div className={styles.problemList}>
                                {loadingProblems && <div className={styles.muted}>Đang tải...</div>}
                                {!loadingProblems && problemsError && (
                                    <div className={styles.errorText}>Không thể tải bài tập. Vui lòng thử lại.</div>
                                )}
                                ...
                                {!loadingProblems && !problemsError && problems.map((p) => (
                                    <div key={p.problemId} className={styles.problemRow}>
                                        <div className={styles.problemRowHeader}>
                                            ...
                                            <span className={`${styles.difficultyBadge} ${difficultyClass[p.difficulty] ?? styles.diffDefault}`}>
                                                {p.difficulty}
                                            </span>
                                        </div>
                                        <button
