...
const Home: React.FC = () => {
  ...
  return (
    /* 1. Xóa sạch pageLayout và mainArea - Chỉ giữ lại content chính */
    <div className={styles.content}>
        ...
        <div className={styles.heroCard}>
          ...
          <div className={styles.progressSection}>
            <div className={styles.progressLabelRow}>
              <span className={styles.progressLabel}>Tiến trình lên Đại Kiện Tướng</span>
        ...
        <div className={`${styles.card} ${styles.leaderCard}`}>
          ...
          <div className={styles.podium}>
            ...
            <div className={styles.podiumItem}>
              ...
              <div className={`${styles.podiumAvatar} ${styles.podiumAvatarSilver}`}>{LEADERBOARD[1].initials}</div>
              <div className={`${styles.podiumBar} ${styles.podiumBarSecond}`} />
              <span className={styles.podiumName}>{LEADERBOARD[1].name}</span>
              <span className={styles.podiumScore}>{LEADERBOARD[1].score}</span>
            </div>
            {/* 1st */}
            <div className={styles.podiumItem}>
              <span className={styles.podiumMedal}>🥇</span>
              <div className={`${styles.podiumAvatar} ${styles.podiumAvatarGold}`}>{LEADERBOARD[0].initials}</div>
              <div className={`${styles.podiumBar} ${styles.podiumBarFirst}`} />
              <span className={styles.podiumName}>{LEADERBOARD[0].name}</span>
              <span className={styles.podiumScore}>{LEADERBOARD[0].score}</span>
            </div>
            {/* 3rd */}
            <div className={styles.podiumItem}>
              <span className={styles.podiumMedal}>🥉</span>
              <div className={`${styles.podiumAvatar} ${styles.podiumAvatarBronze}`}>{LEADERBOARD[2].initials}</div>
              <div className={`${styles.podiumBar} ${styles.podiumBarThird}`} />
              <span className={styles.podiumName}>{LEADERBOARD[2].name}</span>
              <span className={styles.podiumScore}>{LEADERBOARD[2].score}</span>
            </div>
          </div>
      ...
      <div className={styles.bottomRow}>
        ...
        <div className={`${styles.card} ${styles.latestCard}`}>
          ...
          {loadingProblems && <span className={styles.muted}>Đang tải...</span>}
          {!loadingProblems && problems.map((p) => (
            <div key={p.problemId} className={styles.problemRow} onClick={() => navigate(`/problems/${p.problemId}`)}>
        ...
        <div className={`${styles.card} ${styles.popularCard}`}>
          ...
          {POPULAR.map((p, i) => (
            <div key={p.id} className={styles.popularItem} onClick={() => navigate(`/problems/${p.id}`)}>
        ...
        <div className={`${styles.card} ${styles.statsCard}`}>
          ...
          <span className={styles.statsMotivation}>Tiếp tục nỗ lực nhé, Bug hunter!</span>
          <div className={styles.statsProgressRow}>
            <div className={styles.statsProgressLabel}>
              <span>Bài đã AC</span>
              <span className={styles.statsProgressLabelRight}>{AC_SOLVED}/{TOTAL_PROBLEMS}</span>
            </div>
          ...
          <div className={styles.statsGrid}>
            <div className={styles.statsGridItem}>
              ...
              <span className={styles.statsGridValue}>{USER_SCORE.toLocaleString()}</span>
            </div>
            <div className={styles.statsGridItem}>
          ...
          <button className={styles.profileBtn} onClick={() => navigate("/statistics")}>
            Xem chi tiết →
          </button>
        </div>
      </div>
